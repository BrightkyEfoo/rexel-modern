...
interface UseBrandPageFiltersParams {
  brandSlug: string;
  priceRange: [number, number];
  baseUrl?: string;
}

export function useBrandPageFilters({ brandSlug, priceRange, baseUrl }: UseBrandPageFiltersParams) {
  ...
  const filters = useMemo((): SearchFilters => {
    ...
    if (searchParams.get('categories')) {
      urlFilters.categories = searchParams.get('categories')?.split(',').map(Number);
    }
  ...
  const updateFilters = useCallback((newFilters: Partial<SearchFilters>) => {
    ...
    const newUrl = `${baseUrl || '/marque'}/${brandSlug}?${params.toString()}`;
    router.replace(newUrl, { scroll: false });
  }, [brandSlug, router, searchParams, baseUrl]);

  // Fonction pour réinitialiser tous les filtres
  const clearFilters = useCallback(() => {
    router.replace(`${baseUrl || '/marque'}/${brandSlug}`, { scroll: false });
  }, [brandSlug, router, baseUrl]);
