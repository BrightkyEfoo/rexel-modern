...
interface FooterProps {
  /**
   * Tableau de booléens pour contrôler l'affichage des 4 sections principales du footer
   * [Logo/Social, Liens rapides, Services, Contact]
   * @default [true, true, true, true]
   */
  sectionsVisible?: [boolean, boolean, boolean, boolean];
}

export function Footer({ sectionsVisible = [true, true, true, true] }: FooterProps) {
  const currentYear = new Date().getFullYear();
  
  // Calculer le nombre de sections visibles pour ajuster la grille
  const visibleSectionsCount = sectionsVisible.filter(Boolean).length;
  const gridColsClass = visibleSectionsCount === 0 
    ? "grid-cols-1" 
    : `grid-cols-1 md:grid-cols-${Math.min(visibleSectionsCount, 4)}`;
